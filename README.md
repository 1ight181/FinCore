...
FinCore — сервис на Python/Sanic с асинхронной работой и PostgreSQL в качестве хранилища. Это тестовое задание для DimaTech Ltd.
...
- `APP__HOST` — хост для запуска приложения (по умолчанию `0.0.0.0`).
- `APP__PORT` — порт для запуска приложения (по умолчанию `8000`).
- `APP__AUTO_RELOAD` — автоперезагрузка при изменении кода (`true` или `false`).
